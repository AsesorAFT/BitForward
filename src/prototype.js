...
class BitForward {
  ...
  deleteProject(projectId) {
    ...
    const projectIndex = this.projects.findIndex(
      p => p.id === projectId && p.owner === this.currentUser.username
    );
    if (projectIndex === -1) {
  ...
  async createForwardContract(contractData) {
    ...
    console.log(
      `📄 Contrato forward creado: ${contractId} en ${blockchain} por ${amount} unidades`
    );
    return contract;
  }
  ...
  async validateContractData(data) {
    ...
    if (amount < this.config.minContractAmount[blockchain]) {
      throw new Error(
        `Cantidad mínima para ${blockchain}: ${this.config.minContractAmount[blockchain]}`
      );
    }
    ...
    if (execDate <= new Date() || execDate > maxDate) {
      throw new Error(
        `Fecha de ejecución debe ser entre hoy y ${this.config.maxContractDuration} días`
      );
    }
  ...
  calculatePortfolio(userId = null) {
    ...
    const userContracts = this.contracts.filter(
      c => c.creatorId === targetUserId || c.counterpartyId === targetUserId
    );
    ...
    portfolio.total = Object.values(portfolio.byBlockchain).reduce(
      (sum, chain) => sum + chain.totalAmount,
      0
    );
  ...
  generateAnalytics() {
    ...
    return {
      ...
      marketData: {
        ...
        averageContractSize:
          contracts.length > 0
            ? contracts.reduce((sum, c) => sum + c.amount, 0) / contracts.length
            : 0,
  ...
  calculateWinRate() {
    const executedContracts = this.contracts.filter(
      c => c.status === 'executed' && c.pnl !== undefined
    );
