...
class RiskAnalyzer {
  ...
  analyzeContractRisk(contract) {
    try {
      ...
      const blockchainModel =
        this.riskModels.get('blockchain')[blockchain] || this.riskModels.get('blockchain').ethereum;
      const contractModel =
        this.riskModels.get('contractType')[contractType] ||
        this.riskModels.get('contractType').standard;
      ...
      riskScore +=
        ...
        blockchainModel.volatilityFactor *
        durationModel.volatilityMultiplier;
  ...
  identifyPortfolioRiskFactors(contracts, diversification) {
    ...
    const dominantBlockchain = Object.entries(blockchains).sort((a, b) => b[1] - a[1])[0];
    ...
    const highVolatilityContracts = contracts.filter(contract => {
      ...
      return (
        volatilityData && (volatilityData.level === 'HIGH' || volatilityData.level === 'VERY_HIGH')
      );
    });
  ...
  updateMarketData(priceData) {
    ...
    if (priceDelta && Math.abs(priceDelta) > 0.05) {
      // >5% cambio
      this.createAlert({
        type: 'price_movement',
  ...
  generateRiskReport(portfolio) {
    ...
    if (Object.keys(blockchains).length === 1) {
      ...
      riskAnalysis.recommendations.push(
        `Diversifique su portfolio con contratos en otras blockchains además de ${blockchain}`
      );
    } else {
    ...
    for (const blockchain in blockchains) {
      ...
      if (
        volatilityData &&
        (volatilityData.level === 'HIGH' || volatilityData.level === 'VERY_HIGH')
      ) {
    ...
    if (soonExpiringContracts.length > 0) {
      ...
      riskAnalysis.recommendations.push(
        'Prepare estrategias para los contratos a punto de expirar'
      );
    }
    ...
    if (riskAnalysis.level === 'HIGH' || riskAnalysis.level === 'VERY_HIGH') {
      riskAnalysis.recommendations.push(
        'Considere reequilibrar su portfolio para reducir el riesgo general'
      );
    } else if (riskAnalysis.level === 'LOW') {
      riskAnalysis.recommendations.push(
        'Su portfolio tiene un nivel de riesgo bajo, lo que puede resultar en rendimientos más bajos'
      );
    }
