...
class PortfolioManager {
  ...
  async fetchUserContracts(userId) {
    ...
    if (typeof window !== 'undefined' && window.bitForward) {
      return window.bitForward.contracts.filter(
  ...
  calculatePerformanceMetrics(userId) {
    ...
    // Calcular cambios
    const weeklyChange = weekSnapshot
      ? this.calculateChange(weekSnapshot.totalValue, currentValue)
      : 0;
    const monthlyChange = monthSnapshot
      ? this.calculateChange(monthSnapshot.totalValue, currentValue)
      : 0;
    const yearlyChange = yearSnapshot
      ? this.calculateChange(yearSnapshot.totalValue, currentValue)
      : 0;
  ...
  analyzeRisk(userId) {
    ...
    const dominantBlockchain = Object.entries(blockchainCounts).sort((a, b) => b[1] - a[1])[0];
  ...
  generateRecommendations(userId) {
    ...
    if (portfolio.summary.riskLevel === 'HIGH' || portfolio.summary.riskLevel === 'VERY_HIGH') {
      recommendations.push({
        ...
        priority: 'high',
        message:
          'Su portfolio tiene un nivel de riesgo alto. Considere reequilibrar con contratos de menor riesgo.',
      });
    }
    ...
    if (portfolio.metrics.monthlyChange < 0) {
      recommendations.push({
        ...
        priority: 'medium',
        message:
          'Su portfolio ha tenido rendimiento negativo en el último mes. Revise su estrategia.',
      });
    }
  ...
  addToWatchlist(userId, item) {
    ...
    // Verificar si ya existe
    const exists = watchlist.items.some(i => i.type === item.type && i.id === item.id);
