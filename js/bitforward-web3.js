...
class BitForwardWeb3 {
  ...
  async initContracts() {
    try {
      ...
      this.contracts.forwardEngine = new ethers.Contract(
        addresses.forwardEngine,
        this.abis.forwardEngine,
        signer
      );
  ...
  async createForward(params) {
    try {
      const { collateral, notionalUSD, expiryDays, leverage, isLong, targetPrice = 0 } = params;
