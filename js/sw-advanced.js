...
const RULES = {
  ...
  isPriceRequest: url => {
    return url.pathname.includes('/price') || url.hostname.includes('coingecko.com');
  },
};
        ...
        return Promise.all(
          cacheNames
            .filter(cacheName => {
              ...
              return (
                cacheName.startsWith('bitforward-') &&
...
self.addEventListener('push', event => {
  ...
  event.waitUntil(self.registration.showNotification(data.title || 'BitForward', options));
});
