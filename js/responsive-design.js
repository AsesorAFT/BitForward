  ...
  class ResponsiveDesign {
    ...
    applyResponsiveEnhancements() {
      ...
      window.dispatchEvent(
        new CustomEvent('bf-responsive-updated', {
    ...
    adjustFontSizes() {
      ...
      if (isSmall) {
        ...
        document
          .querySelectorAll('h1, h2, h3, .card-title, .value-large, .stat-value')
          .forEach(el => {
            el.style.fontSize = '';
          });
      }
    ...
    improveFormElementsOnMobile() {
      ...
      if (isSmall) {
        ...
        document
          .querySelectorAll(
            'input, select, textarea, button, .form-row, .form-group, .form-row > *, .form-group > *'
          )
          .forEach(el => {
            el.style.minHeight = '';
            el.style.minWidth = '';
