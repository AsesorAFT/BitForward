...
class DynamicModuleLoader {
  ...
  getModuleImport(moduleName) {
    const modules = {
      ...
      'ethers': () => import(/* webpackChunkName: "ethers" */ 'https://cdn.jsdelivr.net/npm/ethers@5.7.2/dist/ethers.esm.min.js'),
      ...
      'dashboard-renderer': () => import(/* webpackChunkName: "dashboard-renderer" */ './dashboard-renderer.js'),
      'dashboard-web3': () => import(/* webpackChunkName: "dashboard-web3" */ './dashboard-web3.js'),
  ...
  async loadRouteModules(route) {
    ...
    if (modules) {
      ...
      try {
        await Promise.all(
          modules.map(mod => this.load(mod, { priority: 'high' }))
        );
