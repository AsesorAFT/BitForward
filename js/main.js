...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (typeof notificationSystem !== 'undefined') {
    notificationSystem.info('BitForward v2.0 con Motor de Persistencia cargado exitosamente', {
...
async function handleContractSubmission(event) {
  ...
  if (validationResult.warnings.length > 0) {
    notificationSystem.warning('Advertencias encontradas en el contrato', {
  ...
  try {
    ...
    notificationSystem.success('¡Contrato creado exitosamente!', {
    ...
    if (typeof notificationSystem.contractCreated === 'function') {
      notificationSystem.contractCreated(response.data.id, payload.blockchain, payload.amount);
    }
    ...
    if (response.data.shareUrl) {
      notificationSystem.info(`Enlace para compartir: ${response.data.shareUrl}`, {
    ...
    notificationSystem.error('Error al crear el contrato', {
...
function crearContratoBitcoin(amount, counterparty, executionDate) {
  ...
  if (typeof bitForward !== 'undefined') {
    const contract = bitForward.createForwardContract(
      'bitcoin',
      amount,
      counterparty,
      executionDate,
...
function crearContratoSolana(amount, counterparty, executionDate) {
  ...
  // Simular proceso de creación
  showSuccess(
    `¡Contrato Solana creado exitosamente!\n\nDetalles:\n- Cantidad: ${amount} SOL\n- Contraparte: ${counterparty}\n- Fecha de ejecución: ${formatDate(executionDate)}`
  );
...
function viewContractDetails(contractId) {
  ...
  alert(
    `Funcionalidad de detalles del contrato ${contractId} - Por implementar en futuras versiones.`
  );
}
