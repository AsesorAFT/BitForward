...
class DashboardCustomizer {
  ...
  setupDashboard() {
    ...
    widgets.forEach((widget, index) => {
      ...
      if (!widget.querySelector('.widget-header')) {
        const title = widget.querySelector('.card-title, .panel-title, h2, h3') || {
          textContent: 'Widget',
        };
  ...
  updateWidgetToggles(modal) {
    ...
    widgets.forEach(widget => {
      const widgetId = widget.id;
      const widgetTitle =
        widget.querySelector('.widget-title')?.textContent ||
    ...
    if (themeSettings.backgroundEffect) {
      modal.querySelectorAll('.effect-option').forEach(option => {
        option.classList.toggle(
          'selected',
          option.dataset.effect === themeSettings.backgroundEffect
        );
      });
    }
  ...
  saveCustomizationSettings(modal) {
    ...
    const backgroundEffect =
      modal.querySelector('.effect-option.selected')?.dataset.effect || 'none';
