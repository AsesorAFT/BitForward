...
class BitForwardStore {
  ...
  removeContract(contractId) {
    const contracts = this.state.contracts.filter(contract => contract.id !== contractId);
  ...
  removeNotification(id) {
    const notifications = this.state.notifications.filter(notif => notif.id !== id);
  ...
  persistState() {
    try {
      ...
      localStorage.setItem(this.persistConfig.key, JSON.stringify(stateToPersist));
    } catch (error) {
  ...
  get computed() {
    return {
      ...
      activeContractsCount: () => {
        return this.state.contracts.filter(contract => contract.status === 'active').length;
      },
