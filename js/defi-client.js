...
class BitForwardDeFiClient {
  ...
  handleStrategyExecuted(data) {
    this.showNotification(
      `⚡ Estrategia ejecutada: ${data.strategy} - Hash: ${data.transactionHash.slice(0, 8)}...`
    );
    this.refreshProtocolData();
  }
  ...
  handleLoanOpened(data) {
    this.showNotification(
      `🏦 Nuevo préstamo: ${data.loanId} - Colateral: ${data.collateralAmount} ${data.collateralAsset}`
    );
  ...
  async refreshProtocolData() {
    await Promise.all([this.getProtocolStats(), this.getVaultInfo(), this.getActiveLoans()]);
  }
