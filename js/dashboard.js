...
class BitForwardEnterprise {
  ...
  updateRealTimePrices() {
    ...
    priceElements.forEach(element => {
      ...
      element.style.background =
        fluctuation > 0 ? 'rgba(0, 212, 170, 0.1)' : 'rgba(232, 67, 147, 0.1)';
      setTimeout(() => {
        element.style.background = '';
      }, 1000);
    });
  ...
  handleProductAction(event) {
    ...
    if (!this.isUserAuthenticated()) {
      const message = window.i18n
        ? window.i18n.t('notification.login.required')
        : 'Debes iniciar sesión para acceder a este producto';
  ...
  renderContractsTable() {
    ...
    tableBody.innerHTML = this.contractsData
      .map(
        contract => `
  ...
  updateForwardSummary(modal) {
    ...
    if (marginEl)
      marginEl.textContent = `$${(requiredMargin || 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}`;
    ...
    const liqPrice =
      direction === 'long' ? currentPrice * (1 - movePct) : currentPrice * (1 + movePct);
    const liqEl = modal.querySelector('#liquidation-price');
    if (liqEl)
      liqEl.textContent = `$${liqPrice.toLocaleString(undefined, { maximumFractionDigits: 2 })}`;
  }
...
function updateEstimatedFees() {
  ...
  const hurdle = balance * 0.1;
  const perfBase = Math.max(0, pnl - hurdle);
  const perfFee = perfBase > 0 ? perfBase * 0.1 : 0;
