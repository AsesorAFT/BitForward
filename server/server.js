...
import { generalLimiter, authLimiter, apiLimiter, contractLimiter } from './middleware/rateLimiter.js';
import { sanitizeBody, handleValidationErrors, validateContractCreation, validateAuthentication, validateTransaction } from './middleware/validation.js';
...
class BitForwardServer {
  constructor() {
    ...
    this.initializeSentry(); // Inicializar Sentry primero
  ...
  /**
     * Inicializa Sentry para el monitoreo de errores del backend.
     */
  initializeSentry() {
    Sentry.init({
      ...
      release: `bitforward-enterprise@${process.env.npm_package_version}`,
    });
    console.log('📈 Sentry for backend monitoring initialized.');
  }
  ...
  initializeMiddleware() {
    ...
    // Sanitización y validación global básica
    this.app.use(express.json({ limit: '10mb' }));
    this.app.use(express.urlencoded({ extended: true }));
    this.app.use(sanitizeBody); // Sanitiza todas las peticiones

    // Aplicar rate limiting general a toda la API
    this.app.use('/api/', generalLimiter);

    // Sistema de seguridad mejorada (Helmet + XSS + CSRF)
  ...
  initializeRoutes() {
    ...
    // API routes con persistencia SQLite y seguridad por ruta
    this.app.use('/api/auth', authLimiter, authRoutes);
    this.app.use('/api/auth/wallet', authLimiter, require('./routes/walletAuth')); // Wallet authentication
    this.app.use('/api/contracts', contractLimiter, contractRoutes);
    this.app.use('/api/stats', apiLimiter, statsRoutes);
    this.app.use('/api/lending', apiLimiter, lendingRoutes);
  ...
  initializeErrorHandling() {
    // Sentry: The error handler must be registered before any other error middleware and after all controllers
