...
class ValidationService {
  ...
  setupCustomValidators() {
    return {
      ...
      financialAmount: Joi.number().positive().precision(8).max(1000000).messages({
  ...
  get contractCreationSchema() {
    return Joi.object({
      blockchain: Joi.string().valid('bitcoin', 'ethereum', 'solana').required().messages({
      ...
      contractType: Joi.string().valid('standard', 'american', 'european').default('standard'),
  ...
  validateFinancialLimits(amount, blockchain) {
    ...
    if (amount < limit.min) {
      throw ErrorFactory.validation(
        'amount',
        `Amount too small. Minimum: ${limit.min} ${blockchain.toUpperCase()}`
      );
    }
    ...
    if (amount > limit.max) {
      throw ErrorFactory.validation(
        'amount',
        `Amount too large. Maximum: ${limit.max} ${blockchain.toUpperCase()}`
      );
    }
