...
class BitForwardValidator {
  ...
  static validateBlockchain(blockchain) {
    ...
    if (!config.CONTRACTS.SUPPORTED_BLOCKCHAINS.includes(blockchain)) {
      errors.push(
        `Blockchain no soportada: ${blockchain}. Blockchains válidas: ${config.CONTRACTS.SUPPORTED_BLOCKCHAINS.join(', ')}`
      );
    }
  ...
  static validateExecutionDate(executionDate) {
    ...
    const maxDate = new Date(
      now.getTime() + config.CONTRACTS.MAX_EXECUTION_DAYS * 24 * 60 * 60 * 1000
    );

    if (date < minDate) {
      errors.push(
        `Fecha de ejecución debe ser al menos ${config.CONTRACTS.MIN_EXECUTION_HOURS} horas en el futuro`
      );
    }

    if (date > maxDate) {
      errors.push(
        `Fecha de ejecución no puede ser más de ${config.CONTRACTS.MAX_EXECUTION_DAYS} días en el futuro`
      );
    }
  ...
  static validateStrikePrice(strikePrice) {
    ...
    if (numericPrice > 10000000) {
      // 10 millones como límite arbitrario
