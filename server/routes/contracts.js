...
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    const contracts = await db('contracts').where('user_id', userId).orderBy('created_at', 'desc');
    ...
    const contractsWithMetrics = contracts.map(contract => {
      ...
      // Calcular P&L
      const unrealizedPnL =
        contract.status === 'active' ? (currentPrice - strikePrice) * amount : 0;
...
router.get('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    const contract = await db('contracts').where({ id: contractId, user_id: userId }).first();
    ...
    const unrealizedPnL = contract.status === 'active' ? (currentPrice - strikePrice) * amount : 0;
    ...
    const contractDetails = {
      ...
      metrics: {
        ...
        unrealizedPnLPercentage:
          strikePrice > 0 ? ((currentPrice - strikePrice) / strikePrice) * 100 : 0,
        daysToExpiration,
        contractAge,
        isExpiring: daysToExpiration <= 7 && daysToExpiration > 0,
        isExpired: daysToExpiration <= 0,
...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    console.log(
      `📊 Detalles: ${amount} ${asset} @ $${strikePrice} (exp: ${expDate.toISOString().split('T')[0]})`
    );
