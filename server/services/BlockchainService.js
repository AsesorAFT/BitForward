...
class BlockchainService {
  ...
  async initializeContracts() {
    ...
    const STRATEGY_EXECUTOR_ADDRESS = process.env.STRATEGY_EXECUTOR_ADDRESS || '0x2345678901234567890123456789012345678901';

    this.contracts.vault = new ethers.Contract(VAULT_ADDRESS, vaultABI, this.wallet);
    this.contracts.strategyExecutor = new ethers.Contract(STRATEGY_EXECUTOR_ADDRESS, strategyExecutorABI, this.wallet);
  ...
  async setupEventListeners() {
    ...
    this.contracts.strategyExecutor.on('HedgeExecuted', (assetIn, assetOut, amountIn, amountOut, user, event) => {
      this.handleStrategyEvent('HedgeExecuted', { assetIn, assetOut, amountIn, amountOut, user }, event);
    });

    this.contracts.strategyExecutor.on('LoanOpened', (loanId, collateralAsset, debtAsset, collateralAmount, debtAmount, event) => {
      this.handleStrategyEvent('LoanOpened', { loanId, collateralAsset, debtAsset, collateralAmount, debtAmount }, event);
    });
  ...
  async getVaultInfo() {
    try {
      const [tvl, supportedAssetCount, authorizedAdapterCount] = await this.contracts.vault.getVaultInfo();
      return {
        success: true,
        data: {
  ...
  async openLoan(collateralAsset, debtAsset, collateralAmount, debtAmount) {
    try {
      ...
      const loanOpenedEvent = receipt.logs.find(log =>
